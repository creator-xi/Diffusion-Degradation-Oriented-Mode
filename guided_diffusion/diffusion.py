...
from guided_diffusion.adapter import Adapter
...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        if self.config.model.type == 'simple':
            ...
            if name != 'celeba_hq':
                ...
                ckpt = os.path.join("../DDNM", self.args.exp, "logs/celeba/celeba_hq.ckpt")
            ...
            if self.config.model.class_cond:
                ckpt = os.path.join("../DDNM", self.args.exp, 'logs/imagenet/%dx%d_diffusion.pt' % (
                ...
                ckpt = os.path.join("../DDNM", self.args.exp, "logs/imagenet/256x256_diffusion_uncond.pt")
                if not os.path.exists(ckpt):
                    download(
            ...
            if self.config.model.class_cond:
                ckpt = os.path.join("../DDNM", self.args.exp, 'logs/imagenet/%dx%d_classifier.pt' % (
                self.config.data.image_size, self.config.data.image_size))
                if not os.path.exists(ckpt):
        ...
        # ------ degradation -----
        if args.simplified:
            # get degradation operator -simplied
            ...
            if args.deg =='colorization':
                ...
                self.A = torch.nn.AdaptiveAvgPool2d((256//scale,256//scale))
                self.Ap = lambda z: MeanUpsample(z,scale)
            elif args.deg =='inpainting':
                # loaded = np.load("exp/inp_masks/mask.npy")
                loaded = np.load(f"exp/inp_masks/{args.mask}.npy")
                ...
                self.A = lambda z: A3(A2(A1(z)))
                self.Ap = lambda z: A1p(A2p(A3p(z)))
            elif args.deg =='diy':
            ...
            if deg == 'cs_walshhadamard':
                ...
                jpeg_qf = args.deg_scale
                from functions.svd_operators import JPEG
                self.A_funcs = JPEG(config.data.channels, self.config.data.image_size, self.device, jpeg_qf)
            elif deg == 'jpeg_t':
                jpeg_qf = args.deg_scale
                from functions.svd_operators import JPEG_test
                self.A_funcs = JPEG_test(config.data.channels, self.config.data.image_size, self.device, jpeg_qf)
            else:
        ...
        # ----- adapter ------
        if args.A_type == "adapter" in args.A_type:
            self.adapter = Adapter(nums_rb=args.nums_rb, args=self.args)
            if args.adap_pth != "empty":
                self.adapter.load_pth(args.adap_pth, self.device)
        ...
        # compute parameter
        # print('parameters_count:',count_parameters(self.adapter))
        # print('parameters_count:',count_parameters(self.model))
    ...
    def sample(self, simplified):
        if simplified:
            ...
            self.simplified_ddnm_plus(self.model, self.cls_fn)
        else:
            ...
            self.svd_based_ddnm_plus(self.model, self.cls_fn)
        ...
        for x_orig, classes in pbar:
            ...
            for i in range(len(Apy)):
                tvu.save_image(
                    ...
                    os.path.join(self.args.image_folder, f"Apy/{idx_so_far + i}.png")
                )
                tvu.save_image(
                    inverse_data_transform(config, x_orig[i]),
                    os.path.join(self.args.image_folder, "gt", f"{idx_so_far + i}.png")
                )
                
            # init x_T
            if args.init_x == 'pure_noise':
                x = torch.randn(
                    y.shape[0],
            ...
            elif args.init_x == 'Apy':
                x = Apy
            elif args.init_x == 'noisy_Apy':
                x = 0.5*Apy + 0.5*torch.randn(
                    y.shape[0],
                    config.data.channels,
                    config.data.image_size,
                    config.data.image_size,
            ...
            with torch.no_grad():
                ...
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    if j < i: # normal sampling 
                        ...
                        if args.A_type == "math":
                            x0_t_hat = x0_t - lambda_t*self.Ap(self.A(x0_t) - y)
                        elif args.A_type == "adapter":
                            x0_t_hat = x0_t - self.adapter(self.Ap(self.A(x0_t) - y), t)
                    ...
                    if args.visual_every_sample:
                        x = xs[-1]
                        x = [inverse_data_transform(config, xi) for xi in x]
                        tvu.save_image(x[0], os.path.join(self.args.image_folder, f"{idx_so_far + j}_{0}_{i}.png"))
            ...
            tvu.save_image(
                x[0], os.path.join(self.args.image_folder, "res", f"{idx_so_far + j}.png")
            )
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            if 'color' in deg:
                ...
                pass
            elif 'jpeg' in deg or 'jpeg_t' in deg:
                pass
            else:
                hw = hwc / 3
                h = w = int(hw ** 0.5)
            ...
            for i in range(len(Apy)):
                ...
                # tvu.save_image(
                #     inverse_data_transform(config, y[i].reshape(config.data.channels, self.config.data.image_size//4,
                #                                     self.config.data.image_size//4)),
            ...
            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddnm
                    x, _ = ddnm_diffusion(x, model, self.adapter, self.betas, self.args.eta, self.A_funcs, y, cls_fn=cls_fn, classes=classes, config=config, args=self.args)
                else: # noisy case, turn to ddnm+
                    x, _ = ddnm_plus_diffusion(x, model, self.adapter, self.betas, self.args.eta, self.A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config, args=self.args)

            x = [inverse_data_transform(config, xi) for xi in x]

            os.makedirs(os.path.join(self.args.image_folder, "res"), exist_ok=True)

            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, "res", f"{idx_so_far + j}.png")
                )
    ...
    def q_sample(self, x_start, t, noise=None):     # add noise, output x_noisy
    ...
    def my_sample(self, images):                  # input a image, return result x0, train do test
        args, config = self.args, self.config
        args.sigma_y = 2 * args.sigma_y #to account for scaling to [-1,1]
        sigma_y = args.sigma_y
            ...
            if args.time_travel:
                    ...
                    if j < i: # normal sampling 
                        ...
                        if args.simplified:
                            ...
                            if args.A_type == "math":
                                ...
                                # pdb.set_trace()
                                x0_t_hat = x0_t - self.adapter(self.A_funcs.A_pinv(
                                    self.A_funcs.A(x0_t.reshape(x0_t.size(0), -1)) - y.reshape(y.size(0), -1)
                                ).reshape(*x0_t.size()), t)
                ...
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    if args.simplified:
                        ...
                        elif args.A_type == "adapter_new2.0":
                            x0_t_hat = x0_t - self.adapter(self.A_funcs.A_pinv(
                                self.A_funcs.A(x0_t.reshape(x0_t.size(0), -1))
                            ).reshape(*x0_t.size()) - self.A_funcs.A_pinv(y.reshape(y.size(0), -1)).reshape(*x0_t.size()), t)
                        elif args.A_type == "adapter_new":

                            x0_t_hat = x0_t - self.A_funcs.A_pinv(
                                self.A_funcs.A(x0_t.reshape(x0_t.size(0), -1))
                            ).reshape(*x0_t.size()) + self.A_funcs.A_pinv(y.reshape(y.size(0), -1)).reshape(*x0_t.size())
...
def extract_into_tensor(a, t, x_shape):
    b, *_ = t.shape
    out = a.gather(-1, t)      # from a get the index-t data, acutually at
