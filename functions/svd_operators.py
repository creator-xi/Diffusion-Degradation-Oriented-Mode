...
class JPEG:
    ...
    def __init__(self, channels, img_dim, device, jpeg_qf):
        self.channels = channels
        self.img_dim = img_dim
        self.device = device
    ...
    def A(self, vec):
        temp = vec.reshape(vec.shape[0], self.channels, self.img_dim, self.img_dim)
        # temp = torch.zeros(vec.shape[0], self.img_dim, self.img_dim, self.channels, device=vec.device)
        # temp[:, :, :, :] = vec.clone().reshape(vec.shape[0], self.img_dim, self.img_dim, self.channels)
        # temp = temp.permute(0, 3, 1, 2)
        x_luma, x_chroma = jpeg_encode(temp, self.jpeg_qf)
        # pdb.set_trace()
        # cat x_luma&x_chroma
        x_chroma_repeated = torch.zeros(x_luma.shape[0], 2, x_luma.shape[2], x_luma.shape[3], device = x_luma.device)
    ...
    def A_pinv(self, vec):
        temp = vec.reshape(vec.shape[0], self.channels, self.img_dim, self.img_dim)
        x_luma = temp[:,:1,:,:]
        x_chroma = temp[:,1:,0::2,0::2]
        x = jpeg_decode([x_luma, x_chroma], self.jpeg_qf)
        # pdb.set_trace()
        # x = x.permute(0, 2, 3, 1)
        return x.reshape(x.shape[0],-1)
...
class DR2(A_functions):
    def __init__(self, channels, img_dim, N, device):
        self.channels = channels
        self.img_dim = img_dim
        self.down_N = N
        shape = (1, channels, img_dim, img_dim)
        shape_d = (1, channels, img_dim // self.down_N, img_dim // self.down_N)
        self.down = Resizer(shape, 1 / self.down_N).to(device)
        self.up = Resizer(shape_d, self.down_N).to(device)

    def A(self, vec):
        temp = vec.reshape(vec.shape[0], self.channels, self.img_dim, self.img_dim)
        return self.down(temp)
    
    def A_pinv(self, vec):
        temp = vec.reshape(vec.shape[0], self.channels, self.img_dim // self.down_N, self.img_dim // self.down_N)
        return self.up(temp)
