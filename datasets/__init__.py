...
#             )
    ...
    elif config.data.dataset == 'Set5':
        # only use validation dataset here
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'Set5', 'imagenet'),
                    os.path.join(args.exp, 'datasets', 'Set5', 'Set5.txt'),
                    image_size=config.data.image_size,
                    normalize=False)
        test_dataset = dataset
    
    elif config.data.dataset == 'Set14':
        # only use validation dataset here
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'Set14', 'imagenet'),
                    os.path.join(args.exp, 'datasets', 'Set14', 'Set14.txt'),
                    image_size=config.data.image_size,
                    normalize=False)
        test_dataset = dataset
    
    elif config.data.dataset == 'Kodak24':
        # only use validation dataset here
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'Kodak24', 'imagenet'),
                    os.path.join(args.exp, 'datasets', 'Kodak24', 'Kodak24.txt'),
                    image_size=config.data.image_size,
                    normalize=False)
        test_dataset = dataset
            
    elif config.data.dataset == 'RealSRSet':
        # only use validation dataset here
        from datasets.imagenet_subset import ImageDataset
        dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'RealSRSet', 'imagenet'),
                    os.path.join(args.exp, 'datasets', 'RealSRSet', 'RealSRSet.txt'),
                    image_size=config.data.image_size,
                    normalize=False)
        test_dataset = dataset
    else:
